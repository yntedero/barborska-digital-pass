...
<template>
  <div class="relative flex items-center justify-center group">
    ...
    <div
      ...
      role="img"
      :aria-label="state === 'validated' ? `Stop ${stopId} - GPS verified` : state === 'partial' ? `Stop ${stopId} - Visited` : state === 'viewed' ? `Stop ${stopId} - Viewed` : `Stop ${stopId} - Not visited`"
    >
      ...
      <template v-if="state === 'validated'">
        ...
        <div
          ...
          aria-hidden="true"
        >
          <div class="stamp-shimmer absolute inset-0 bg-gradient-to-r from-transparent via-white/25 to-transparent -translate-x-full" />
        </div>
      </template>
