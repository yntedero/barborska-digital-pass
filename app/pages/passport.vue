...
<template>
  <div class="pb-24">
    ...
    <!-- Progress section -->
    <div class="px-4 py-4">
      <div class="bg-white dark:bg-(--color-sand-800) rounded-xl border border-(--color-sand-200) dark:border-(--color-sand-700) p-5 overflow-hidden relative">
        ...
        <div class="relative">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-sm font-bold text-(--color-gold-500)">
              {{ passport.progress }}%
            </span>
          </div>
          <div class="w-full h-3 bg-(--color-sand-100) dark:bg-(--color-sand-700) rounded-full overflow-hidden">
            <div
          ...
          <div class="flex items-center gap-4 mt-4">
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full bg-gradient-to-br from-(--color-gold-400) to-(--color-gold-600) shadow-sm" />
              <span class="text-xs text-(--color-sand-500) dark:text-(--color-sand-400)">
                {{ t('passport.validated') }}: <strong class="text-(--color-sand-700) dark:text-(--color-sand-200)">{{ passport.validated }}</strong>
              </span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full bg-amber-500 shadow-sm" />
              <span class="text-xs text-(--color-sand-500) dark:text-(--color-sand-400)">
                {{ t('passport.visited') }}: <strong class="text-(--color-sand-700) dark:text-(--color-sand-200)">{{ passport.visited }}</strong>
              </span>
            </div>
          </div>
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span v-else>
                {{ passport.validated }}/{{ passport.STAMPS_FOR_CERTIFICATE }} {{ t('passport.needed') }}
              </span>
            </div>
          </div>
        </div>
    ...
    <div class="px-4 space-y-5">
      ...
      >
        ...
        <div class="flex items-center gap-3 mb-3">
          <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-(--color-gold-100) to-(--color-gold-200) dark:from-(--color-gold-900) dark:to-(--color-gold-950) flex items-center justify-center shadow-sm">
            <span class="text-xs font-bold text-(--color-gold-600) dark:text-(--color-gold-400)">{{ stage.id }}</span>
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-(--color-sand-800) dark:text-(--color-sand-100) truncate">
              {{ stage.from }} → {{ stage.to }}
            </p>
        ...
        <div class="flex flex-wrap gap-3">
          ...
          >
            ...
            <span class="text-[9px] text-(--color-sand-400) dark:text-(--color-sand-500) text-center max-w-12 truncate">
              {{ stop.name }}
            </span>
          </div>
        </div>
    ...
    <!-- Save note callout -->
    <div class="px-4 mt-5">
      <div class="flex items-start gap-2 px-4 py-3 rounded-xl bg-(--color-sand-100) dark:bg-(--color-sand-900) border border-(--color-sand-200) dark:border-(--color-sand-800)">
        <UIcon
          name="i-lucide-info"
      ...
      <template #content>
        ...
        >
          <VisitorStampIcon
            ...
            class="mx-auto mb-3"
          />
          <h3 class="font-heading text-lg font-bold text-(--color-sand-800) dark:text-(--color-sand-100)">
